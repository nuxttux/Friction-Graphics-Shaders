// Light Rays2 — with angle cropping + rotation
#version 330 core
...
uniform float seed1;
uniform float seed2;
uniform float angleMin;     // minimum angle (radians)
uniform float angleMax;     // maximum angle (radians)
uniform float rotation;     // rotate angular sector (radians)
...
void main() {
    ...
    // compute angle of this fragment relative to origin
    float fragAngle = atan(dir.y, dir.x) - rotation;

    // wrap angle to [-PI, PI]
    if(fragAngle > 3.14159265) fragAngle -= 6.2831853;
    if(fragAngle < -3.14159265) fragAngle += 6.2831853;

    // crop rays to angle range
    if (fragAngle < angleMin || fragAngle > angleMax) {
        fragColor = vec4(0.0);
        return;
    }
